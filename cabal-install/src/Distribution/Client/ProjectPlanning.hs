        ...
        where
          ...
          setupComponent
            ...
            where
              ...
              dep_pkgs = CD.setupDeps solverPkgLibDeps >>= elaborateLibSolverId mapDep
          ...
          buildComponent (cc_map, lc_map, exe_map) comp =
              ...
              $ do
                ...
                cc0 <-
                  toConfiguredComponent
                    ...
                    (Map.unionWith Map.union ((fmap . fmap) (\(WithStage _ a) -> a) external_lib_cc_map) cc_map)
                    -- ^ lib_dep_map
                    (Map.unionWith Map.union ((fmap . fmap) (\(WithStage _ a) -> a) external_exe_cc_map) cc_map)
                    -- ^ exe_dep_map
                    comp

                let do_ cid = -- filled in later in pruneInstallPlanPhase2)
                              (annotatedIdToConfiguredId (ci_ann_id cid), False)
            ...
            where
              ...
              external_lib_cc_map :: Map PackageName (Map ComponentName (WithStage (AnnotatedId ComponentId)))
              external_lib_cc_map =
                Map.fromListWith Map.union $
                  map mkCCMapping external_lib_dep_pkgs
              
              external_exe_cc_map :: Map PackageName (Map ComponentName (WithStage (AnnotatedId ComponentId)))
              external_exe_cc_map =
                Map.fromListWith Map.union $
                  map mkCCMapping external_exe_dep_pkgs
              
              external_lc_map :: Map ComponentId (OpenUnitId, ModuleShape)
              external_lc_map =
                Map.fromList $
                  map mkShapeMapping $
              ...
              compPkgConfigDependencies =
                ...
                | PkgconfigDependency pn _ <- PD.pkgconfigDepends (Cabal.componentBuildInfo comp)
                ]
      ...
      elaborateSolverToPackage
        ...
        comps = do
          ...
          where
            ...
            pkgLibDependencies :: CD.ComponentDeps [(ConfiguredId, Bool)]
            pkgLibDependencies =
              buildComponentDeps (filter (isExternal . fst) . compLibDependencies)
            
            pkgExeDependencies :: CD.ComponentDeps [WithStage ConfiguredId]
            pkgExeDependencies =
              buildComponentDeps (filter isExternal' . compExeDependencies)
            
            pkgExeDependencyPaths :: CD.ComponentDeps [(WithStage ConfiguredId, FilePath)]
            pkgExeDependencyPaths =
              buildComponentDeps (filter (isExternal' . fst) . compExeDependencyPaths)

            -- TODO: Why is this flat?
            pkgPkgConfigDependencies :: [(PkgconfigName, Maybe PD.PkgconfigVersion)]
            pkgPkgConfigDependencies =
...
mkCCMapping
  :: ElaboratedPlanPackage
  -> (PackageName, Map ComponentName (WithStage (AnnotatedId ComponentId)))
mkCCMapping =
  InstallPlan.foldPlanPackage
    ( \(WithStage stage ipkg) ->
        ( packageName ipkg
        , Map.singleton
            (ipiComponentName ipkg)
            -- TODO: libify
            ( WithStage stage $ AnnotatedId
          ...
          , case elabPkgOrComp elab of
              ElabComponent comp ->
                case compComponentName comp of
                  Nothing -> Map.empty
                  Just n -> Map.singleton n (WithStage (elabStage elab) (mk_aid n))
              ElabPackage _ ->
                Map.fromList $
                  map
                    (\comp -> let cn = Cabal.componentName comp in (cn, WithStage (elabStage elab) (mk_aid cn)))
                    (Cabal.pkgBuildableComponents (elabPkgDescription elab))
          )
...
pruneInstallPlanPass2 pkgs =
  ...
  where
    ...
    hasReverseLibDeps :: Set (WithStage UnitId)
    hasReverseLibDeps =
      Set.fromList
        [ depid
        | InstallPlan.Configured pkg <- pkgs
    ...
    hasReverseExeDeps :: Set (WithStage UnitId)
    hasReverseExeDeps =
      Set.fromList
        [ depid
        | InstallPlan.Configured pkg <- pkgs
